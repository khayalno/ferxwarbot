  ...
  try:
    ...
    for chat_id, content in lists_db.items():
      data_to_save[str(chat_id)] = {
          ...
          "hijri_date": content.get("hijri_date", ""),
...
def create_final_list(message, teacher_name, assistant_name, msg_ids):
  ...
  # وەرگرتنی بەرواری کۆچی لە بەرواری زاینییەوە
  hijri_date_obj = Gregorian(now.year, now.month, now.day).to_hijri()
  hijri_date = f"{hijri_date_obj.day}/{hijri_date_obj.month}/{hijri_date_obj.year}"
...
📅 بەرواری زاینی: {data['greg_date']}  |  {hijri} ک
