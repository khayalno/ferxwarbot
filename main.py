...
TOKEN = "8876884764:AAEW8zgMlPhEASVV8SLN4tQS9gqDzgMsHPU"
  ...
  if os.path.exists(DB_FILE):
    try:
      with open(DB_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
        for chat_id in data:
          data[chat_id]["completed"] = set(
              str(x) for x in data[chat_id].get("completed", [])
          )
    ...
    for chat_id, content in lists_db.items():
      data_to_save[str(chat_id)] = {
          ...
          "listeners": [
              ...
              for l in content.get("listeners", [])
          ],
          "leaves": [
              {"id": str(v["id"]), "name": v["name"]}
              for v in content.get("leaves", [])
          ],
          "is_closed": content.get("is_closed", False),
          "registration_stopped": content.get("registration_stopped", False),
...
def handle_stoplist(message):
  if message.chat.type in ["group", "supergroup"]:
    chat_id = str(message.chat.id)
    user_id = message.from_user.id

    if not is_admin(message.chat.id, user_id):
      bot.reply_to(message, "⚠️ تەنها ئەدمینەکان دەتوانیت ناونووسین ڕابگرن.")
      return

    if chat_id in lists_db:
      lists_db[chat_id]["registration_stopped"] = True
      save_db()

      msg_id = lists_db[chat_id].get("message_id")
...
def handle_closelist(message):
  if message.chat.type in ["group", "supergroup"]:
    ...
    if chat_id in lists_db:
      ...
      lists_db[chat_id]["registration_stopped"] = False
      save_db()
...
def ask_assistant_name(message, msg_ids):
  ...
  msg = bot.send_message(
      message.chat.id,
      f"🎓 مامۆستا: {teacher_name}\n\nئێستا ناوی یاریدەدەر بنووسە:",
  )
  msg_ids.append(msg.message_id)
  bot.register_next_step_handler(
      msg, lambda m: ask_subject_name(m, teacher_name, msg_ids)
  )
...
def ask_subject_name(message, teacher_name, msg_ids):
  ...
  msg = bot.send_message(
      message.chat.id,
      f"👤 یاریدەدەر: {assistant_name}\n\nئێستا **ناوی وانە** بنووسە:",
  )
  msg_ids.append(msg.message_id)
  bot.register_next_step_handler(
      msg,
      lambda m: create_final_list(m, teacher_name, assistant_name, msg_ids),
  )
...
📋 لیستی فێرخوازان:\n"""
  ...
  if data.get("is_closed", False):
    text += "\n🔒 **[ ئەم لیستە داخراوە ]**"
  elif data.get("registration_stopped", False):
    text += "\n⚠️ **[ ناونووسین ڕاگیراوە ]**"
...
def build_keyboard(chat_id):
  chat_id = str(chat_id)
  data = lists_db.get(chat_id, {})
  if data.get("is_closed", False):
    return None

  reg_stopped = data.get("registration_stopped", False)
  markup = types.InlineKeyboardMarkup(row_width=2)

  if not reg_stopped:
    markup.add(
        types.InlineKeyboardButton(
            "📝 ناوم تۆمار بکە", callback_data="act_register"
        ),
  ...
  markup.add(
      types.InlineKeyboardButton(
          "📩 ناردنی لیست بۆ تایبەتی ئەدمین", callback_data="act_send_private"
      )
  )
  return markup
...
def send_updated_list(chat_id, message_id=None):
  ...
  try:
    msg = bot.send_message(
        int(chat_id), text, reply_markup=markup, parse_mode="Markdown"
    )
...
def handle_buttons(call):
  ...
  # وەرگرتنی یوزەرنەیم یان ناوی ئەکاونت ئەگەر یوزەرنەیم نەبوو
  if call.from_user.username:
    user_identifier = f"@{call.from_user.username}"
  else:
    user_identifier = call.from_user.first_name
  ...
  def remove_user_from_all():
    data["students"] = [
        s for s in data["students"] if str(s["id"]) != user_id
    ]
    data["completed"].discard(user_id)
    data["listeners"] = [
        l for l in data["listeners"] if str(l["id"]) != user_id
    ]
    data["leaves"] = [l for l in data["leaves"] if str(l["id"]) != user_id]

  if action == "act_register":
    if data.get("registration_stopped", False):
      safe_answer_cb(
          call.id, "⚠️ ناونووسین لەلایەن ئەدمینەوە ڕاگیراوە.", show_alert=True
      )
      return
    remove_user_from_all()
    data["students"].append({"id": user_id, "name": user_identifier})
    save_db()
    safe_answer_cb(call.id, "یوزەرنەیمەکەت تۆمار کرا.")
    ...
    remove_user_from_all()
    data["listeners"].append({"id": user_id, "name": user_identifier})
    save_db()
    safe_answer_cb(call.id, "گوێگربوون تۆمار کرا.")
  elif action == "act_leave":
    remove_user_from_all()
    data["leaves"].append({"id": user_id, "name": user_identifier})
    save_db()
    ...
    try:
      bot.send_message(
          int(user_id), f"📥 **لیستی ئەمرۆ:**\n\n{list_text}", parse_mode="Markdown"
      )
      safe_answer_cb(call.id, "✅ بۆ چاتی تایبەتیت ڕەوانە کرا.")
    except Exception:
      safe_answer_cb(
          call.id, "⚠️ سەرەتا بۆتەکە لە تایبەتی /start بکە.", show_alert=True
      )
    return
