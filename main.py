...
def setup_bot_commands():
    try:
        commands = [
            ...
            types.BotCommand("stoplist", "وەستاندنی لیستەکە"),
...
def handle_stoplist(message):
    if message.chat.type in ['group', 'supergroup']:
        ...
        try:
            chat_member = bot.get_chat_member(int(chat_id), int(user_id))
            if chat_member.status not in ['administrator', 'creator']:
                bot.reply_to(message, "🚫 تەنها ئەدمین دەتوانێت لیستەکە ڕابوەستێنێت.")
                return
        except Exception:
            return
        ...
        if chat_id in lists_db:
            ...
            save_db()
            send_updated_list(chat_id, message_id=lists_db[chat_id].get('message_id'))
            bot.reply_to(message, "🛑 لیستەکە ڕاوەستا.")
        else:
            bot.reply_to(message, "⚠️ هیچ لیستێکی چالاک بوونی نییە.")
...
def ask_assistant_name(message):
    ...
    bot.register_next_step_handler(msg, lambda m: ask_subject_name(m, teacher_name))

def ask_subject_name(message, teacher_name):
    assistant_name = message.text.strip()
    msg = bot.reply_to(message, f"📚 ئەوەی ناوی یاریدەدەر: {assistant_name}\n\nئێستا **ناوی وانە** بنووسە:")
    bot.register_next_step_handler(msg, lambda m: create_final_list(m, teacher_name, assistant_name))

def create_final_list(message, teacher_name, assistant_name):
...
def build_keyboard(is_closed=False):
    ...
    if not is_closed:
        ...
        markup.add(btn_register, btn_listen)
    ...
    markup.add(btn_delete, btn_read)
...
def handle_buttons(call):
    ...
    if action == "act_register":
        if data.get('is_closed', False):
            safe_answer_cb(call.id, "🔒 ناونووسین ڕاوەستاوە.", show_alert=True)
            return
        ...
        if data.get('is_closed', False):
            safe_answer_cb(call.id, "🔒 گوێگرتن ڕاوەستاوە.", show_alert=True)
            return
        remove_user_from_all()
